...
void send_usb_req(int sockfd, USBIP_RET_SUBMIT * usb_req, char * data, unsigned int size, unsigned int status)
{
	...
	usb_req->direction=0x0;
        ...
        if (send (sockfd, (char *)usb_req, sizeof(USBIP_RET_SUBMIT), 0) != (int)sizeof(USBIP_RET_SUBMIT))
        {
          printf ("send error : %s \n", strerror (errno));
          return;   /* dropped connection: bail out of this URB, do not kill the process */
        };

        if(size > 0)
        {
           if (send (sockfd, data, size, 0) != (int)size)
           {
             printf ("send error : %s \n", strerror (errno));
             return;
           };
...
#if defined(LINUX) || defined(__APPLE__)
        printf("%016llX\n",(unsigned long long)usb_req->setup);
#else
        printf("%016I64X\n",usb_req->setup);
#endif
...
/* Stop flag + live fds, so the server can be torn down from another thread (iOS lifecycle).
 * Closing the fds unblocks a thread parked in accept()/recv(). */
static volatile int g_usbip_stop = 0;
...
usbip_run (const USB_DEVICE_DESCRIPTOR *dev_dsc)                                /* simple TCP server */
{
  ...
  if (bind (listenfd, (sockaddr *) & serv, sizeof (serv)) < 0)
    {
      printf ("bind error : %s \n", strerror (errno));
      close (listenfd); g_usbip_listenfd = -1;
      return;
    };
  ...
  while (!g_usbip_stop)
    {
      ...
      if (
        ...
        {
          if (g_usbip_stop) break;        /* usbip_stop() closed listenfd */
